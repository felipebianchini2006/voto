...
export const VotingBoothPage: React.FC = () => {
  ...
  const tokenMutation = useMutation({
    ...
    onError: (err: any) => {
      setError(err.response?.data?.message || 'Erro ao solicitar token');
    },
  });
  ...
  const handleNext = () => {
    ...
    if (activeStep === 0) {
      if (!externalId.trim()) {
        setError('Por favor, informe seu ID externo');
        return;
      }
        ...
        {activeStep === 0 && (
          <Box sx={{ maxWidth: 400, margin: 'auto', textAlign: 'center' }}>
            ...
            <Button
              ...
              startIcon={tokenMutation.isPending ? <CircularProgress size={20} /> : null}
            >
              {tokenMutation.isPending ? 'Solicitando...' : 'Solicitar Token'}
            </Button>
          </Box>
        )}
        ...
        {activeStep === 1 && (
          <Box>
            ...
            <Box sx={{ mt: 4, display: 'flex', justifyContent: 'flex-end' }}>
              ...
              <Button variant="contained" onClick={handleNext} disabled={!selectedCandidate}>
                Continuar
              </Button>
            </Box>
          </Box>
          ...
          <Box sx={{ maxWidth: 500, margin: 'auto', textAlign: 'center' }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'center', gap: 2 }}>
              ...
              <Button
                ...
                startIcon={voteMutation.isPending ? <CircularProgress size={20} /> : null}
              >
                {voteMutation.isPending ? 'Registrando...' : 'CONFIRMAR VOTO'}
              </Button>
            </Box>
          </Box>
        )}
